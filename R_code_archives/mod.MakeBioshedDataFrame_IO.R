...
scenarios <- c("Basecase, all energy crops", "2% yield inc.", "3% yield inc.", 
               "4% yield inc.")
feeds <- c("Corn stover", "Wheat straw", "Biomass sorghum", "Switchgrass", 
           "Miscanthus")
    ...
    for (range in c(50)) {
      ...
      for (scenario in scenarios) {
        ...
        for (price in prices) {
          ...
          # clean up df
          drop.cols <- c("All_Feedstocks", "long.1", "lat.1")
          res <- res[,!(names(res) %in% drop.cols)]
...
saveRDS(res.1, "output.bioshed.data.RDS")
