...
# RETURNS:
# a list of lists. In each sublist the first element is RID and second is
...
CalcBiosheds <- function(hubs.data, crop, edges.data, constraint = "distance", 
                         max.dist = NULL,
                         max.time = NULL) {
...
#  no_cores = 30
  ...
  # set RID sequence
  rid.res <- foreach (RID = hubs.data$RID, .export = c("CheckIfInRange"),
    ...
    try({                                             
      ...
      if (!is.null(cand.counties)) {
        cids.in.range <- 
          foreach (f = cand.counties$cid[1:length(cand.counties$cid)],
                   .export = c("CalcDriveRoute", "RID", "wgs84.crs"),
      ...
      saveRDS(in.range, paste0("../output/bin/RID_", 
                               RID, "_", crop, 
                               "_bioshed_", max.dist, "mi.RDS"))
